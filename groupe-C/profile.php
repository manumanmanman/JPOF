...
if (!isset($_SESSION["user"])) { // il n'est pas logué, on redirige vers la page d'accueil
 ...
 <div class="col-12  px-4 mb-5">
     ...
     <h4 class="text-danger mt-5 pb-2 px-3">Activités dans vos favoris</h4>
 </div>
...
if(isset($_SESSION["user"])){
   ...
   $userfav = "SELECT * FROM favorites 
   LEFT OUTER JOIN activities ON favorites.activity_id = activities.activity_id
   ...
   WHERE user_token = '$fuserid'";

    


   $results = $conn->query($userfav); 
...
<div class="col-12 col-lg-4 col-md-6 mb-5 mx-0 pb-5 d-flex justify-content-center">
    ...
    <div class="coeur"><a class="remove btn btn-danger btn-block text-white" href="#" data-activity="<?php echo ($result["activity_id"]); ?>"><span class="texte">Retirer des favoris</span></a></div>
    </div>
...
<a href="profile-inscrit.php?fuserid=<?php echo $fuserid?>" type="button" class="btn btn-secondary btn-lg btn-block mb-5">Activités auxquelles vous êtes inscrit</a>
</div>
